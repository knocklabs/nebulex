defmodule Knock.Nebulex.Telemetry.CacheStatsCounterHandler do
  @moduledoc """
  Telemetry handler for aggregating cache stats; it relies on the default
  `Knock.Nebulex.Adapters.Common.Info` implementation based on Erlang counters.
  See `Knock.Nebulex.Adapters.Common.Info.Stats`.
  """
  ...
  defp update_stats(%{
         command: action,
         result: {:error, %Knock.Nebulex.KeyError{reason: :expired}},
  ...
  defp update_stats(%{
         command: action,
         result: {:error, %Knock.Nebulex.KeyError{reason: :not_found}},
         adapter_meta: %{stats_counter: ref}
       })
       when action in [:fetch, :take, :ttl, :has_key?] do
