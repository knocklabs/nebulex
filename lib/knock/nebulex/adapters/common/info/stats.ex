defmodule Knock.Nebulex.Adapters.Common.Info.Stats do
  @moduledoc """
  Stats implementation using [Erlang counters][erl_counters].

  Adapters are directly responsible for implementing the `Knock.Nebulex.Adapter.Info`
  ...
  See the `Knock.Nebulex.Adapters.Local` adapter and `Knock.Nebulex.Adapters.Common.Info`
  for more information about the usage.

  [erl_counters]: https://erlang.org/doc/man/counters.html
  """

  alias Knock.Nebulex.Telemetry.CacheStatsCounterHandler
  ...
  Any adapter using `Knock.Nebulex.Adapters.Common.Info` implementation must call
  this init function in the `c:Knock.Nebulex.Adapter.init/1` callback and include
  the returned counter within the adapter metadata under the key
  `:stats_counter`. See the `Knock.Nebulex.Adapters.Nil` for example.

  ## Example

      Knock.Nebulex.Adapters.Common.Info.Stats.init([:telemetry, :prefix])
  ...
  ## Examples
      ...
      Knock.Nebulex.Adapters.Common.Info.Stats.incr(stats_counter, :hits)

      Knock.Nebulex.Adapters.Common.Info.Stats.incr(stats_counter, :writes, 10)

      Knock.Nebulex.Adapters.Common.Info.Stats.incr(stats_counter, [:misses, :deletions])
  ...
  ## Examples
      ...
      Knock.Nebulex.Adapters.Common.Info.Stats.count(stats_counter)
  ...
  ## Examples
      ...
      Knock.Nebulex.Adapters.Common.Info.Stats.count(stats_counter, :hits)
