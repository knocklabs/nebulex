if Code.ensure_loaded?(Decorator.Define) do
  defmodule Knock.Nebulex.Caching.Decorators do
    @moduledoc """
    ...
    #{Knock.Nebulex.Caching.Options.shared_options_docs()}
    ...
    > See ["Dynamic caches"](`m:Knock.Nebulex.Cache#module-dynamic-caches`)
    ...
    `use Knock.Nebulex.Caching`. For example:
    ...
    globally (when defining the caching usage via `use Knock.Nebulex.Caching`),
    ...
    definition (e.g., `use Knock.Nebulex.Caching, cache: MyCache`) or a specific
    ...
    strategy given by `Knock.Nebulex.Caching.Decorators.generate_key/1`, which is
    ...
    ### Best practices
      ...
      * If you need access to the function arguments, use the
        **decorator context** instead of capturing:
        ...
        ```elixir
        def match_fun(result, %Knock.Nebulex.Caching.Decorators.Context{args: [arg]}) do
    ...
    **`:raise`** - Raises on cache errors
    ...
    import Knock.Nebulex.Utils, only: [get_option: 5]
    import Record
    ...
    the value can be:
      ...
      * A dynamic cache spec created with the macro
        [`dynamic_cache/2`](`Knock.Nebulex.Caching.dynamic_cache/2`).
    ...
    @type keyref_spec() ::
            record(:keyref, cache: Knock.Nebulex.Cache.t(), key: any(), ttl: timeout() | nil)
    ...
    the value can be:
      ...
      * An explicit key reference definition `t:keyref_spec/0`. It must be
        created using the macro [`keyref/2`](`Knock.Nebulex.Caching.keyref/2`).
    ...
    #{Knock.Nebulex.Caching.Options.cacheable_options_docs()}
    ...
    cache `RedisCache`; the macro [`keyref/2`](`Knock.Nebulex.Caching.keyref/2`)
    ...
    #{Knock.Nebulex.Caching.Options.cache_evict_options_docs()}
    ...
    > `Knock.Nebulex.Adapters.Local` adapter, which uses ETS match specifications.
    ...
    cached entries matching the given tag.
        ...
        defp query_for_tag(%{args: [tag]} = _context) do
          # Assuming we are using the `Knock.Nebulex.Adapters.Local` adapter and the
    ...
    assume you are using the `Knock.Nebulex.Adapters.Local` adapter):
    ...
    The `Knock.Nebulex.Adapters.Local` adapter provides powerful strategies for
    ...
    [`Knock.Nebulex.Adapters.Local.QueryHelper`](https://hexdocs.pm/nebulex_local/Knock.Nebulex.Adapters.Local.QueryHelper.html)
    documentation.
    ...
    [cache put](`c:Knock.Nebulex.Cache.put/3`) operation.
    ...
    `Knock.Nebulex.Caching.dynamic_cache/2` instead.
    ...
    `Knock.Nebulex.Caching.keyref/2` instead.
