defmodule Knock.Nebulex.Cache.Supervisor do
  @moduledoc false

  use Supervisor

  import Knock.Nebulex.Cache.Options
  ...
  def start_child({mod, fun, args}, name, meta) do
    with {:ok, pid} <- apply(mod, fun, args) do
      ...
      :ok = Knock.Nebulex.Cache.Registry.register(self(), name, meta)
  ...
  defp maybe_set_bypass_adapter(adapter, opts) do
    case Keyword.fetch!(opts, :bypass_mode) do
      true -> Knock.Nebulex.Adapters.Nil
      false -> adapter
    end
  end
end
