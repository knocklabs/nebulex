defmodule Knock.Nebulex.Adapter.Queryable do
  @moduledoc """
  ...
  `Knock.Nebulex.Adapters.Local` adapter supports
  ...
  See [Knock.Nebulex.Cache.get_all/2 options](`m:Knock.Nebulex.Cache#c:get_all/2-options`)
  ...
  the selected entry fields to return can be found in the `query_meta`.
   ...
   Finally, `opts` is a keyword list of options given to the `Knock.Nebulex.Cache`
  ...
  It is used on `c:Knock.Nebulex.Cache.get_all/2`, `c:Knock.Nebulex.Cache.count_all/2`,
  and `c:Knock.Nebulex.Cache.delete_all/2`.
  """
  @callback execute(adapter_meta(), query_meta(), opts()) ::
              Knock.Nebulex.Cache.ok_error_tuple([any()] | non_neg_integer())
  ...
  See `c:Knock.Nebulex.Cache.stream/2`.
  """
  @callback stream(adapter_meta(), query_meta(), opts()) ::
              Knock.Nebulex.Cache.ok_error_tuple(Enumerable.t())
end
