defmodule Knock.Nebulex.Adapter do
  @moduledoc """
  Specifies the minimal API required from adapters.
  """

  import Knock.Nebulex.Utils, only: [is_nebulex_wrappable_exception: 1]

  alias Knock.Nebulex.Cache.Options
  alias Knock.Nebulex.Error, as: NbxError
  ...
  @type nbx_error() :: Knock.Nebulex.Error.t() | Knock.Nebulex.KeyError.t()
  ...
  this function based on the dynamic cache:
      ...
      Knock.Nebulex.Adapter.lookup_meta(cache.get_dynamic_cache())

  """
  @spec lookup_meta(atom() | pid()) :: adapter_meta()
  defdelegate lookup_meta(name_or_pid), to: Knock.Nebulex.Cache.Registry, as: :lookup
  ...
  **NOTE:** Internal purposes only.
  ...
  ## Examples
      ...
      iex> Knock.Nebulex.Adapter.handle_command_response({:ok, "ok"})
      {:ok, "ok"}

      iex> Knock.Nebulex.Adapter.handle_command_response(:ok)
      :ok

      iex> Knock.Nebulex.Adapter.handle_command_response(
      ...>   {:error, %Knock.Nebulex.Error{reason: :error}}
      ...> )
      {:error, %Knock.Nebulex.Error{reason: :error}}

      iex> Knock.Nebulex.Adapter.handle_command_response({:error, :error})
      {:error, %Knock.Nebulex.Error{reason: :error}}

      iex> Knock.Nebulex.Adapter.handle_command_response({:error, %RuntimeError{}})
      {:error, %Knock.Nebulex.Error{reason: %RuntimeError{}}}
