defmodule Knock.Nebulex.Cache do
  @moduledoc """
  ...
  #{Knock.Nebulex.Cache.Options.compile_options_docs()}
  ...
  [`Knock.Nebulex.Adapters.Local`][local_adapter] for more information.
  Despite this, the following configuration values are shared
  across all adapters:

  #{Knock.Nebulex.Cache.Options.start_link_options_docs()}

  [local_adapter]: https://hexdocs.pm/nebulex_local/Knock.Nebulex.Adapters.Local.html
  ...
  #{Knock.Nebulex.Cache.Options.runtime_shared_options_docs()}
  ...
  All Nebulex caches emit the following events:
    ...
    * `[:knock_nebulex, :cache, :init]` - It is dispatched whenever a cache starts.
  ...
  The following are the events you should expect from Knock.Nebulex. All examples
  ...
  Example event data:
      ...
      %{
        ...
        metadata: %{
          ...
          extra_metadata: %{user_id: 123},
          kind: :error,
          reason: %Knock.Nebulex.KeyError{key: "key", reason: :not_found},
          stacktrace: [...]
        }
      }
  ...
  `Knock.Nebulex.Caching.Decorators` module documentation for attributes like
  ...
  @type event_listener() :: Knock.Nebulex.Event.listener()

  @typedoc "Proxy type to a cache event  filter"
  @type event_filter() :: Knock.Nebulex.Event.filter()

  @typedoc "Proxy type for generic Nebulex error"
  @type nbx_error_reason() :: Knock.Nebulex.Error.t()

  @typedoc "Fetch error reason"
  @type fetch_error_reason() :: Knock.Nebulex.KeyError.t() | nbx_error_reason()
  ...
  defmacro __using__(opts) do
    quote do
      ...
      if Knock.Nebulex.Adapter.Queryable in behaviours do
        unquote(queryable_defs())
      end

      if Knock.Nebulex.Adapter.Transaction in behaviours do
        unquote(transaction_defs())
      end

      if Knock.Nebulex.Adapter.Info in behaviours do
        unquote(info_defs())
      end

      if Knock.Nebulex.Adapter.Observable in behaviours do
        unquote(event_defs())
      end
    end
  end
  ...
  defp prelude(opts) do
    quote do
      @behaviour Knock.Nebulex.Cache

      {otp_app, adapter, behaviours, opts} =
        Knock.Nebulex.Cache.Supervisor.compile_config(unquote(opts))
  ...
  defp base_defs do
    quote do
      ...
      @impl true
      def config do
        {:ok, config} = Knock.Nebulex.Cache.Supervisor.runtime_config(__MODULE__, @otp_app, [])
      ...
      def start_link(opts \\ []) do
        Knock.Nebulex.Cache.Supervisor.start_link(__MODULE__, @otp_app, @adapter, opts)
      end
  ...
  defp queryable_defs do
    quote do
      alias Knock.Nebulex.Cache.Queryable
  ...
  defp transaction_defs do
    quote do
      alias Knock.Nebulex.Cache.Transaction
  ...
  defp info_defs do
    quote do
      alias Knock.Nebulex.Cache.Info
  ...
  defp event_defs do
    quote do
      alias Knock.Nebulex.Cache.Observable
  ...
  @callback __adapter__() :: Knock.Nebulex.Adapter.t()
  ...
  `MyApp.Cache`. But you can change the default cache at compile-time:
      ...
      use Knock.Nebulex.Cache, default_dynamic_cache: :cache_name
  ...
  `Knock.Nebulex.KeyError` if the cache does not contain `key`,
  `Knock.Nebulex.Error` otherwise.
  ...
  ## Examples
      ...
      # Key not found error
      iex> {:error, %Knock.Nebulex.KeyError{key: "bar"} = e} = MyCache.fetch("bar")
      iex> e.reason
      :not_found
  ...
  @doc """
  Same as `c:fetch/2` but raises `Knock.Nebulex.KeyError` if the cache doesn't contain
  `key` or `Knock.Nebulex.Error` if another error occurs while executing the command.
  ...
  @doc """
  Same as `c:fetch/3` but raises `Knock.Nebulex.KeyError` if the cache doesn't contain
  `key` or `Knock.Nebulex.Error` if another error occurs while executing the command.
  """
  @doc group: "KV API"
  ...
  #{Knock.Nebulex.Cache.Options.put_options_docs()}
  ...
  #{Knock.Nebulex.Cache.Options.write_options_docs()}
  ...
  #{Knock.Nebulex.Cache.Options.replace_options_docs()}
  ...
  If there's an error with executing the command, `{:error, reason}`
  is returned. `reason` is the cause of the error and can be
  `Knock.Nebulex.KeyError` if the cache does not contain `key` or
  `Knock.Nebulex.Error` otherwise.
  ...
  ## Examples
      ...
      iex> {:error, %Knock.Nebulex.KeyError{key: :a} = e} = MyCache.take(:a)
      iex> e.reason
      :not_found
  ...
  ## Examples
      ...
      iex> {:error, %Knock.Nebulex.KeyError{key: :c} = e} = MyCache.ttl(:c)
      iex> e.reason
      :not_found
  ...
  #{Knock.Nebulex.Cache.Options.update_counter_options_docs()}
  ...
  When the function returns an error, the value is not cached:
      ...
      {:error, %Knock.Nebulex.Error{reason: "not found"}}

      iex> MyCache.fetch("user")
      {:error, %Knock.Nebulex.KeyError{key: "user"}}  # Key was not cached
  ...
  Returns the unwrapped value on success, or raises `Knock.Nebulex.Error` if the
  ...
  ## Examples
      ...
      ** (Knock.Nebulex.Error) fetch_or_store command failed with reason: :error
  ...
  **Use `fetch_or_store/3` when:**
  ...
  function's return value (even if it's an error tuple) is cached and returned.
      ...
      ** (Knock.Nebulex.Error) fetch_or_store command failed with reason: :not_found
  ...
  May raise `Knock.Nebulex.QueryError` if query validation fails.
  ...
  #{Knock.Nebulex.Cache.QuerySpec.options_docs()}
  ...
  ### Query examples for `Knock.Nebulex.Adapters.Local` adapter

  The `Knock.Nebulex.Adapters.Local` adapter supports **"ETS Match Spec"** as query
  ...
  `Knock.Nebulex.Adapters.Local` adapter provides several advanced features for more
  ...
  [Local Adapter documentation](https://hexdocs.pm/nebulex_local/Knock.Nebulex.Adapters.Local.html).
  """
  @doc group: "Query API"
  ...
  #{Knock.Nebulex.Cache.Options.stream_options_docs()}
  ...
  We can provide the keys to lock when using the `Knock.Nebulex.Adapters.Local`
  ...
  provided by `Knock.Nebulex.Adapters.Common.Info`.
  ...
  Listeners can only raise `Knock.Nebulex.Error` exception. Caching implementations
  must catch any other exception from a listener, then wrap and reraise it as
  a `Knock.Nebulex.Error` exception.

  ## Options

  #{Knock.Nebulex.Cache.Options.observable_options_docs()}
