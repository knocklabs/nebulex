defmodule Knock.Nebulex.Cache.KVErrorTest do
  ...
  deftests do
    ...
    describe "KV error" do
      ...
      test "fetch/2", %{cache: cache} = ctx do
        assert {:error, %Knock.Nebulex.Error{module: module, reason: reason}} = cache.fetch(:error)
      ...
      test "get/2", %{cache: cache} = ctx do
        assert {:error, %Knock.Nebulex.Error{module: module, reason: reason}} = cache.get("error")
      ...
      test "delete/2", %{cache: cache} = ctx do
        assert {:error, %Knock.Nebulex.Error{module: module, reason: reason}} = cache.delete("error")
      ...
      test "take/2", %{cache: cache} = ctx do
        assert {:error, %Knock.Nebulex.Error{module: module, reason: reason}} = cache.take("error")
      ...
      test "has_key?/1 command failed", %{cache: cache} = ctx do
        assert {:error, %Knock.Nebulex.Error{module: module, reason: reason}} = cache.has_key?("error")
