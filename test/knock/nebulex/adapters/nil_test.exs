defmodule Knock.Nebulex.Adapters.NilTest do
  ...
  describe "kv" do
    ...
    test "fetch", %{cache: cache} do
      assert {:error, %Knock.Nebulex.KeyError{key: "foo"}} = cache.fetch("foo")
    end
    ...
    test "take", %{cache: cache} do
      assert {:error, %Knock.Nebulex.KeyError{key: "foo"}} = cache.take("foo")
    end
    ...
    test "ttl", %{cache: cache} do
      assert {:error, %Knock.Nebulex.KeyError{key: "foo"}} = cache.ttl("foo")
    end
