defmodule Knock.Nebulex.Adapters.CacheTest do
  use ExUnit.Case, async: true

  # Cache API test cases
  use Knock.Nebulex.CacheTestCase

  import Knock.Nebulex.CacheCase, only: [setup_with_dynamic_cache: 2, t_sleep: 1]

  setup_with_dynamic_cache Knock.Nebulex.TestCache.Cache, __MODULE__
  ...
  describe "error" do
    test "because cache is stopped", %{cache: cache, name: name} do
      ...
      for fun <- ops do
        assert_raise Knock.Nebulex.CacheNotFoundError, ~r/unable to find cache: #{inspect(name)}/, fun
      end
    end
  end
  ...
  describe "KV:" do
    ...
    test "get_and_update fails because cache is not started", %{cache: cache, name: name} do
      ...
      assert_raise Knock.Nebulex.CacheNotFoundError, ~r/unable to find cache: #{inspect(name)}/, fn ->
    ...
    test "incr with ttl", %{cache: cache} do
      ...
      assert {:error, %Knock.Nebulex.KeyError{key: :counter_with_ttl}} = cache.fetch(:counter_with_ttl)
    ...
    test "fetch_or_store! raises if an error occurs", %{cache: cache} do
      assert_raise Knock.Nebulex.Error, ~r"fetch_or_store command failed with reason: :error", fn ->
        cache.fetch_or_store!("lazy", fn -> {:error, :error} end)
      end
  ...
  describe "transaction" do
    test "aborted", %{name: name, cache: cache} do
      ...
      assert_raise Knock.Nebulex.Error, ~r/transaction aborted\n\nError metadata:/, fn ->
        {:error, %Knock.Nebulex.Error{} = reason} =
          cache.transaction(
