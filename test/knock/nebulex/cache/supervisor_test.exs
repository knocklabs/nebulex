defmodule Knock.Nebulex.Cache.SupervisorTest do
  ...
  alias Knock.Nebulex.{Adapter, Telemetry}
  alias Knock.Nebulex.TestCache.Cache
  ...
  describe "compile_config/1" do
    test "error: missing :otp_app option" do
      assert_raise NimbleOptions.ValidationError, ~r"required :otp_app option not found", fn ->
        Knock.Nebulex.Cache.Supervisor.compile_config(adapter: TestAdapter)
      end
    end

    test "error: missing :adapter option" do
      assert_raise NimbleOptions.ValidationError, ~r"required :adapter option not found", fn ->
        Knock.Nebulex.Cache.Supervisor.compile_config(otp_app: :knock_nebulex)
      end
    end
    ...
    test "error: adapter was not compiled" do
      ...
      assert_raise NimbleOptions.ValidationError, msg, fn ->
        Knock.Nebulex.Cache.Supervisor.compile_config(otp_app: :knock_nebulex, adapter: TestAdapter)
      end
    end

    test "error: adapter doesn't implement the required behaviour" do
      msg =
        "invalid value for :adapter option: Knock.Nebulex.Cache expects the option value " <>
          "Knock.Nebulex.Cache.SupervisorTest.MyAdapter to list Knock.Nebulex.Adapter as a behaviour"
    ...
    test "error: invalid value for :adapter option" do
      ...
      assert_raise NimbleOptions.ValidationError, msg, fn ->
        Knock.Nebulex.Cache.Supervisor.compile_config(otp_app: :knock_nebulex, adapter: 123)
      end
    end
  end
  ...
  describe "start_link/1" do
    ...
    test "starts cache with custom adapter" do
      defmodule CustomCache do
        use Knock.Nebulex.Cache,
          otp_app: :knock_nebulex,
          adapter: Knock.Nebulex.TestCache.AdapterMock
      end
    ...
    test "starts cache with [bypass_mode: true]" do
      ...
      assert Adapter.lookup_meta(Cache).adapter == Knock.Nebulex.Adapters.Nil
    ...
    test "emits telemetry event upon cache start" do
      with_telemetry_handler [[:knock_nebulex, :cache, :init]], fn ->
        {:ok, _} = Cache.start_link(name: :telemetry_test)

        assert_receive {[:knock_nebulex, :cache, :init], _, %{cache: Cache, opts: opts}}
