...
All previously built-in adapters (`Knock.Nebulex.Adapters.Local`,
`Knock.Nebulex.Adapters.Partitioned`, `Knock.Nebulex.Adapters.Replicated`, and
`Knock.Nebulex.Adapters.Multilevel`) have been moved to separate repositories.
...
For example, if you're using `Knock.Nebulex.Adapters.Partitioned` with a custom
...
-   primary_storage_adapter: Knock.Nebulex.Adapters.Local
+   adapter_opts: [primary_storage_adapter: Knock.Nebulex.Adapters.Local]
end
```
...
The most significant change is in the [Cache API](`Knock.Nebulex.Cache`). Nebulex v3
...
in core (`Knock.Nebulex.Adapter.Transaction`).
...
The `Knock.Nebulex.Adapter.Stats` behaviour was removed. Therefore, the
...
information specification keys they want. See `c:Knock.Nebulex.Cache.info/2` and the
...
Therefore, the `Knock.Nebulex.Adapter.Persistence` behaviour was removed, so
...
Query API expects a ["Query Spec"](`c:Knock.Nebulex.Cache.get_all/2#query-specification`)
...
See [Cache API Docs](`Knock.Nebulex.Cache`) for more information and examples.
...
The `Knock.Nebulex.Caching.KeyGenerator` behaviour was removed. You can use an
...
the `:default_key_generator` option must be provided to `use Knock.Nebulex.Caching`.
...
Then, add it to the modules using `use Knock.Nebulex.Caching`:
...
some of the common options globally when using `use Knock.Nebulex.Caching`.
...
See [`Knock.Nebulex.Caching` options](`m:Knock.Nebulex.Caching#module-compilation-time-options`)
...
> **_See ["Caching Decorators"](Knock.Nebulex.Caching.Decorators.html) for more information._**
